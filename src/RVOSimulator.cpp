...
namespace RVO {
	...
	size_t RVOSimulator::addAgent(const Vector2 &position, const float const_alpha, const float a, const float b,
						const float c, const float d, const float kappa, const float epsilon, const float delta, const float max_noise)
	{
		if (defaultAgent_ == NULL) {
			return RVO_ERROR;
		}

		Agent *agent = new Agent(this, max_noise);
		...
		agent->const_alpha_ = const_alpha;
		agent->a_ = a;
		agent->b_ = b;
		agent->c_ = c;
		agent->d_ = d;
		agent->kappa_ = kappa;
		agent->epsilon_ = epsilon;
		agent->delta_ = delta;
		

		agent->id_ = agents_.size();
		// agent->generator.seed(agent->id_);
	...
	size_t RVOSimulator::addAgent(const Vector2 &position, float neighborDist, size_t maxNeighbors, float timeHorizon, float timeHorizonObst, float radius, float maxSpeed, const Vector2 &velocity,
								  const float const_alpha, const float a, const float b, const float c, const float d, const float kappa,
								  const float epsilon, const float delta, const float max_noise)
	{
		Agent *agent = new Agent(this, max_noise);
...
#pragma omp parallel for
	...
	float RVOSimulator::getAlpha(const size_t agentNo, const size_t neighbor) const{
		// Here, in alpha we store -x_i. This function returns x_i
		if (agents_[agentNo]->const_alpha_ > 1.0){
			if (agents_[agentNo]->alphas_.find(neighbor) == agents_[agentNo]->alphas_.end()){
				return agents_[agentNo]->b_ / agents_[agentNo]->d_;
			}
			else{
				return agents_[agentNo]->alphas_[neighbor].alpha;
			}
		}
		else{
			return agents_[agentNo]->const_alpha_;
		}
	}
}
