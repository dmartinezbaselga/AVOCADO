...
cdef extern from "RVOSimulator.h" namespace "RVO":
    cdef cppclass RVOSimulator:
        ...
        size_t addAgent(const Vector2 &position, const float const_alpha, const float a, const float b,
						const float c, const float d, const float kappa, const float epsilon,
                        const float delta, const float max_noise)
        size_t addAgent(const Vector2 & position, float neighborDist,
                        size_t maxNeighbors, float timeHorizon,
                        float timeHorizonObst, float radius, float maxSpeed,
                        const Vector2 & velocity, const float const_alpha, const float a, const float b,
						const float c, const float d, const float kappa, const float epsilon,
                        const float delta, const float max_noise)
        ...
        float getAlpha(const size_t agentNo, const size_t neighbor) const
...
cdef class PyRVOSimulator:
    ...
    def addAgent(self, tuple pos, neighborDist=None,
                 ...
                 velocity=None, const_alpha=0.5, a=0.1, b=0.0,
				 c=1.0, d=0.1, kappa=1.0, epsilon=2.0, delta=0.1, max_noise=0.0):
        ...
        if neighborDist is None:
            agent_nr = self.thisptr.addAgent(c_pos, const_alpha, a, b, c, d, kappa, epsilon, delta, max_noise)
        else:
            ...
            agent_nr = self.thisptr.addAgent(c_pos, neighborDist,
                                             ...
                                             c_velocity, const_alpha, a, b, c, d, kappa, epsilon, delta, max_noise)
    ...
    def getAlpha(self, size_t agent_no, size_t agent_nr):
        return self.thisptr.getAlpha(agent_no, agent_nr)
